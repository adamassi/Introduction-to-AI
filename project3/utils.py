...
ID = 211696216  # TODO: change it to your personal ID
...
def accuracy(y: np.array, y_pred: np.array)-> float:
    ...
    # correctPreds = np.equal(y, y_pred)
    # accuracy_val = sum(correctPreds) / len(y)
    # ========================
...
def l2_dist(x1: np.array, x2: np.array):
    ...
    x1_sq = np.sum(np.square(x1), axis=1, keepdims=True)  # Shape: (N1, 1)
    x2_sq = np.sum(np.square(x2), axis=1, keepdims=True).T  # Shape: (1, N2)

    # Step 2: Compute the dot product term, -2 * (x1 @ x2.T).
    cross_term = -2 * np.dot(x1, x2.T)  # Shape: (N1, N2)

    # Step 3: Combine all terms to compute the squared L2 distance matrix.
    dists_sq = x1_sq + x2_sq + cross_term  # Shape: (N1, N2)

    # Step 4: Take the square root of the squared distances to get the L2 distance.
